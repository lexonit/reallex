...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate, user }) => {
  ...
  const PipelineSnapshot = () => {
      if (isLoading) return <Skeleton className="w-full h-full min-h-[200px] rounded-xl" />;

      const stats = [
          { label: 'New Lead', count: isAgent ? 8 : 45, color: 'bg-blue-500' },
          { label: 'Contacted', count: isAgent ? 12 : 32, color: 'bg-yellow-500' },
          { label: 'Proposal', count: isAgent ? 5 : 18, color: 'bg-purple-500' },
          { label: 'Negotiation', count: isAgent ? 3 : 10, color: 'bg-orange-500' },
          { label: 'Closed', count: isAgent ? 2 : 15, color: 'bg-emerald-500' },
      ];
      ...
      return (
          <div className="w-full h-full bg-white dark:bg-black border border-neutral-200 dark:border-white/10 rounded-xl p-6 shadow-sm flex flex-col">
              <div className="flex justify-between items-center mb-6">
                 <div>
                    <h3 className="font-bold text-lg">Deals Pipeline</h3>
                    <p className="text-sm text-muted-foreground">Snapshot of active deal stages</p>
                 </div>
                 <Button variant="ghost" size="sm" onClick={() => onNavigate(NavigationTab.LEADS)}>View Pipeline</Button>
              </div>
  ...
  // 5. Rich Activity Feed Widget (Existing)
  const ActivityFeed = () => {
  ...
  const items = [
    ...
    {
        header: <StatCard 
            title={isAgent ? "Active Listings" : "Total Listings"} 
            value={isAgent ? "4" : "48"} 
            icon={Building} 
            trend="neutral" 
            trendValue="2%" 
    ...
    {
        header: <StatCard 
            title={isAgent ? "Pipeline Value" : "Deals Volume"} 
            value={isAgent ? "$4.2M" : "$12.8M"} 
            icon={DollarSign} 
            trend="up" 
            trendValue="8%" 
