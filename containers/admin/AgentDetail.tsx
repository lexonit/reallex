...
import { GET_PROPERTIES_QUERY } from '../../graphql/queries/property.queries';
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const propData = await graphqlRequest(GET_PROPERTIES_QUERY, { filter: { assignedAgentId: agent.id, status: 'Active' } });
                const properties = (propData?.properties || []).map((p: any) => ({
                    id: p._id,
                    address: p.address,
                    price: p.price || 0,
                    ...
                    images: p.images || [],
                    agent: `${p.assignedAgent?.firstName || ''} ${p.assignedAgent?.lastName || ''}`.trim(),
                    type: 'Single Family',
                    ...
                    documents: (p.documents || []).map((d: any, idx: number) => ({ id: `${p._id}-${idx}`, name: d, type: 'PDF', size: '—', url: d })),
                    openHouses: [],
                    location: { lat: p.location?.coordinates?.[1] ?? 0, lng: p.location?.coordinates?.[0] ?? 0 },
                })) as Property[];
                setActiveListings(properties);

                // Fetch closed deals for this agent (stage Closed Won)
                const dealsData = await graphqlRequest(GET_DEALS_QUERY, { filter: { assignedAgentId: agent.id, stage: 'Closed Won' } });
                const deals = (dealsData?.deals || []).map((d: any) => ({
                    ...
                    listedDate: d.createdAt,
                    soldDate: d.closeDate || d.updatedAt || d.createdAt,
                    daysOnMarket: 0,
                    buyerName: d.leadName || '—',
                })) as ClosedDeal[];
              ...
              <div className="grid grid-cols-2 gap-3">
                  ...
                  <Card className="p-4 flex flex-col items-start justify-between bg-card hover:bg-muted/20 transition-colors border-l-4 border-l-orange-500 shadow-sm">
                   ...
                   <Card className="p-4 flex flex-col items-start justify-between bg-card hover:bg-muted/20 transition-colors border-l-4 border-l-emerald-500 shadow-sm">
                      ...
                      <div>
                        <span className="text-2xl font-bold text-foreground">{closedDeals.length}</span>
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mt-1">Closed Deals</p>
                      </div>
                  </Card>
              </div>
                  ...
                  {activeTab === 'INVENTORY' && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {loading && activeListings.length === 0 && (
                            <div className="col-span-full py-12 text-center text-muted-foreground">Loading properties…</div>
                          )}
                  ...
                  {activeTab === 'HISTORY' && (
                      <Card className="border-none shadow-none bg-transparent">
                          <DataTable 
                              data={closedDeals}
                              columns={[
                                  { header: 'Address', accessorKey: 'propertyAddress' as any, className: 'font-medium' },
                                  { header: 'Price', accessorKey: 'salePrice' as any, cell: (d: ClosedDeal) => `$${d.salePrice.toLocaleString()}` },
                                  { header: 'Sold Date', accessorKey: 'soldDate' as any },
                                  { header: 'DOM', accessorKey: 'daysOnMarket' as any, cell: (d: ClosedDeal) => `${d.daysOnMarket} days` },
                              ]}
                              className="bg-card"
                          />
