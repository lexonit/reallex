...
import { BedDouble, Bath, Square, MapPin, FileText } from 'lucide-react';
...
export const PropertyCard: React.FC<PropertyCardProps> = ({ property, onViewDetails, showVendorChip }) => {
  const statusColors = {
    ...
    >
      <Card className="overflow-hidden group hover:shadow-lg transition-all duration-300 border-border/50 bg-card/50 backdrop-blur-sm">
        <div className="relative h-48 w-full overflow-hidden">
          ...
          />
          {showVendorChip && property.vendorId && (
            <div className="absolute top-2 left-2">
              <span className="inline-flex items-center rounded-md px-2 py-1 text-xs font-semibold backdrop-blur-md bg-white/80 text-slate-700 border border-slate-200">
